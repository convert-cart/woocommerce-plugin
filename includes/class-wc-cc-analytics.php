...
<p class="form-row form-row-wide">
	...
	public function get_plugin_info($request)
	{
		...
		// Get plugin version
		$plugin_version = defined('CC_PLUGIN_VERSION') ? CC_PLUGIN_VERSION : 'Unknown';

		// Get webhooks with "convertcart" in their name
		$webhooks = $wpdb->get_results(
			"SELECT webhook_id, name, topic, delivery_url 
			 FROM {$wpdb->prefix}wc_webhooks 
			 WHERE name LIKE '%convertcart%'",
			ARRAY_A
		);
