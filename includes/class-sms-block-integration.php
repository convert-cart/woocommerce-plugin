...
class ConvertCart_SMS_Consent_Block_Integration implements IntegrationInterface {
	...
	public function get_script_data() {
		...
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG === true ) {
			error_log(
				'ConvertCart: SMS consent block data prepared with text: ' . $text
	...
	private function register_block_type() {
		if ( function_exists( 'register_block_type_from_metadata' ) ) {
			...
			register_block_type_from_metadata(
				$block_path,
				array(
					'view_script_handles'   => array( 'convertcart-sms-consent-block-frontend' ),
					'editor_script_handles' => array( 'convertcart-sms-consent-block' ),
				)
			);
		}
	}
